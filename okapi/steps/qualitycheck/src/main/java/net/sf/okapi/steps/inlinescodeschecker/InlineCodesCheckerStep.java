...
public class InlineCodesCheckerStep extends BasePipelineStep {
	...
	protected Event handleStartBatch(Event event) {
		...
		inlineCodesChecker.startProcess(sourceLocale, targetLocale, fullParams, new ArrayList<Issue>());
		return event;
	}
	
	@Override
	protected Event handleStartSubDocument(Event event) {
		inlineCodesChecker.processStartSubDocument(event.getStartSubDocument());
		return event;
	}
	...
	protected Event handleStartDocument(Event event) {
		...
		inlineCodesChecker.processStartDocument((StartDocument) event.getResource(), null);
		return event;
	}

	@Override
	protected Event handleTextUnit(Event event) {
		inlineCodesChecker.processTextUnit(event.getTextUnit());
		return event;
	}
	...
	protected List<Issue> getIssues() {
		return inlineCodesChecker.getIssues();
	}
}
