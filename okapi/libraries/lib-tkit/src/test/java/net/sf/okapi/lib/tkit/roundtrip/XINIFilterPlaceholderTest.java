...
public class XINIFilterPlaceholderTest {
	...
	@Before
	public void setUp() {
		RoundTripUtils.path = TestUtil.getParentDir(this.getClass(), "/dummy.txt")+"test_xini_placeholder.json";
	}
	...
	public void placeholdersBecomeCodes() {
		...
		//tu Content: phphabc
		ITextUnit tu = FilterTestDriver.getTextUnit(roundTripSerilaizedEvents(helper.toEvents(snippet)), 1);
	...
	public void isolatedPlaceholdersBecomeCodes() {
		...
		ITextUnit tu1 = FilterTestDriver.getTextUnit(roundTripSerilaizedEvents(helper.toEvents(snippet)), 1);
	...
	public void placeholdersBecomePreserved() throws Exception {
		...
		List<Event> eventsSnippet = roundTripSerilaizedEvents(helper.toEvents(snippet));
	...
	private String doStringToEventsAndEventsToXiniForPhTypeTest(String snippet){
		Xini xini = helper.toXini(roundTripSerilaizedEvents(helper.toEvents(snippet)));
		List<Field> field = helper.getFieldsByPageIdAndElementId(xini, 1, 10);
		String segContent = helper.getSegContentBySegId(field.get(0), 0);
		return segContent;
	}
