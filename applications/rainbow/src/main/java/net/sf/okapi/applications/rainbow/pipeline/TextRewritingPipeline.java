...
public class TextRewritingPipeline extends PredefinedPipeline {
	...
	public TextRewritingPipeline () {
		...
		((net.sf.okapi.steps.segmentation.Parameters)stepSeg.getParameters()).setSegmentSource(false);
		((net.sf.okapi.steps.segmentation.Parameters)stepSeg.getParameters()).setSegmentTarget(false);
		((net.sf.okapi.steps.segmentation.Parameters)stepSeg.getParameters()).setCopySource(false);
		addStep(stepSeg);

		LeveragingStep stepLev = new LeveragingStep();
		((net.sf.okapi.steps.leveraging.Parameters)stepLev.getParameters()).setLeverage(false);
		addStep(stepLev);
